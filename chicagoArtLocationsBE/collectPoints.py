...
def getHaversine(
    array,
):  ## Calculate the distance in mi. between two cooridnates (haversine formula)
    distDict = (
        {}
    )  ## {rounded int distance between each coord and reference coordinate: {exact float distance of coord to reference: {station info}}}
    for coord in array:
...
def getMore(dist, lat, lon, arr):
    ...
    if int(dist) in arr:
        for station in arr[int(dist)].values():
            haverDist = haversine(
                (lat, lon), (station["lat"], station["lon"]), unit="mi"
            )
...
def getClosest(lat, lon, arr, lDeviation, rDeviation, results):
    ...
    while len(leftDistances) == 0 and list(arr.keys())[0] <= int(dist) - (
        lDeviation + 1
    ):
    ...
    while len(rightDistances) == 0 and list(arr.keys())[-1] >= int(dist) + (
        rDeviation + 1
    ):
    ...
    set_key(
        dotenv_file, "results", str({i: results[i] for i in list(results.keys())[10:]})
    )
    if len(sys.argv) > 2:
...
results = eval(
    os.environ["results"]
)  ## results needs to be initialized to {} i env file. String dict to dict (json.loads requires results variable to be stored in double quotes, eval is more flexible)

if results == {} and sys.argv[1] == "loadmore":
    print("you need to input coordinates first!")

elif (
    results != {} and len(results.keys()) >= 10 and sys.argv[1] == "loadmore"
):  ## load more
    for result in list(results.keys())[:10]:
        print(
            ...
            + results[result]["name"]
            + "\t"
            + ("has kiosk" if results[result]["has_kiosk"] else "no kiosk")
            + "\n"
        )
    # Write changes to .env file.
    set_key(
