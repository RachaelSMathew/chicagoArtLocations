...
function SearchResults() {
  ...
  const { results, lastResultVisible, isLoadingMore } =
    ...
    <>
      {results && results.length > 0 && (
        ...
        >
          ...
          {lastResultVisible && isLoadingMore && (
            <div className="spinner spinner--load-more" />
          )}
          {!isLoadingMore && (
            <i
              style={{
      ...
      {results && results.length === 0 && (
        <i className="noResults">no results found : (</i>
      )}
    </>
  );
}
